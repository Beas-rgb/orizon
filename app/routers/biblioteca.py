...
def lista(
    ...
    docs = chamar(lambda: listar(db, usuario, projeto_id))
...
def detalhe(
    ...
    return _saida(chamar(lambda: obter(db, usuario, documento_id)))
...
def arquivo(
    ...
    doc, conteudo = chamar(lambda: baixar(db, usuario, documento_id))
    return Response(
        content=conteudo,
        media_type=doc.mime,
        headers={
